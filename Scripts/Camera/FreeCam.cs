...
{
    private const float MAX_DISTANCE = 125f;
    ...
    private bool _FreeCamDisabled = false;
    private Vector3 _StartPos;
    
    private void Awake() 
    {
        LevelManager.freeCam = this;
    }
    private void Start() 
    {
    ...
    void Update()
    {
        if(_FreeCamDisabled) return;
        if(Vector3.Distance(transform.position, Vector3.zero) > MAX_DISTANCE) transform.position = _StartPos; 
    ...
    {
        _FreeCamDisabled = false;
    }
    public void DisableFreeCam()
    {
        _FreeCamDisabled = true;
    }

    private void OnEnable() 
    {
        IDEController.onEnableUI += DisableFreeCam;
        IDEController.onDisableUI += EnableFreeCam;
    }
    void OnDisable()
    {
        StopLooking();
